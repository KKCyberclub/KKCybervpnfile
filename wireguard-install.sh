...
new_client_dns () {
	...
	echo "   5) Quad9"
	echo "   6) AdGuard"
	read -p "DNS server [Press Enter to select 1 by default]: " dns
...
EOF
	...
	echo "   4) Exit"
	read -p "Option [Press Enter to choice default [1]]: " option
	if [ ! -z "$option" ]; then
		until [[ "$option" =~ ^[1-4]$ ]]; do
	...
	case "$option" in
		1|"")
			echo
			echo "Provide a name for the client [Press enter to random]:"
			read -p "Name: " unsanitized_client
			random_client="client"$(shuf -i 2000-65000 -n 1)
			unsanitized_client=$unsanitized_client:$random_client
			# Allow a limited set of characters to avoid conflicts
			client=$(sed 's/[^0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ_-]/_/g' <<< "$unsanitized_client")
			while [[ -z "$client" ]] || grep -q "^# BEGIN_PEER $client$" /etc/wireguard/wg0.conf; do
				echo "$client: invalid name."
				read -p "Name [Press enter to random]: " unsanitized_client
				random_client="client"$(shuf -i 2000-65000 -n 1)
				unsanitized_client=$unsanitized_client:$random_client
				client=$(sed 's/[^0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ_-]/_/g' <<< "$unsanitized_client")
			done
